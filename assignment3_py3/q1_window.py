        ...
        for i in range(len(labels)):
            ...
            for j in range(-window_size, window_size + 1):
                ...
                if ind < 0:
                    ...
                    word = sentence[ind]
                featured_data += word
            windowed_data.append((featured_data, labels[i]))
            # END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            tf.int32, [None, self.config.n_window_features])
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('w', [self.config.n_window_features * self.config.embed_size,
                                  self.config.hidden_size], initializer=tf.contrib.layers.xavier_initializer(seed=1))
        U = tf.get_variable('u', [self.config.hidden_size, self.config.n_classes],
                            initializer=tf.contrib.layers.xavier_initializer(seed=2))
        b1 = tf.get_variable(
            'b1', [self.config.hidden_size, ], initializer=tf.zeros_initializer())
        b2 = tf.get_variable(
