...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        mask = [True] * min(max_length, len(sentence)) + \
            [False] * (max_length - len(sentence))
        sentence = sentence[:max_length] + \
            [zero_vector] * (max_length - len(sentence))
        labels = labels[:max_length] + \
            [zero_label] * (max_length - len(labels))
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            tf.int32, [None, self.max_length, self.config.n_features])
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(
            embeddings, [-1, self.max_length, self.config.n_features * self.config.embed_size])
        # END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        if self.config.cell == "rnn":
            cell = RNNCell(Config.n_features *
                           Config.embed_size, Config.hidden_size)
        elif self.config.cell == "gru":
            cell = GRUCell(Config.n_features *
                           Config.embed_size, Config.hidden_size)
        else:
        ...
        assert preds.get_shape().as_list() == [None, self.max_length, self.config.n_classes], "predictions are not of the right shape. Expected {}, got {}".format(
    ...
    def preprocess_sequence_data(self, examples):
        ...
        examples = featurize_windows(
            examples, self.helper.START, self.helper.END, self.config.window_size)
...
def test_pad_sequences():
    ...
    ret = [
        ([[4, 1], [6, 0], [7, 0], [0, 0]], [
         1, 0, 0, 4], [True, True, True, False]),
        ([[3, 0], [3, 4], [4, 5], [5, 3]], [0, 1, 0, 2], [True, True, True, True])
    ]

    ret_ = pad_sequences(data, 4)
    assert len(ret_) == 2, "Did not process all examples: expected {} results, but got {}.".format(
        2, len(ret_))
    for i in range(2):
...
if __name__ == "__main__":
    ...
    command_parser.add_argument('-dt', '--data-train', type=argparse.FileType(
        'r'), default="data/tiny.conll", help="Training data")
    command_parser.add_argument(
        '-dd', '--data-dev', type=argparse.FileType('r'), default="data/tiny.conll", help="Dev data")
