...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            b_z = tf.get_variable('b_z', [self.state_size, ], initializer=tf.zeros_initializer()) 
            ...
            b_r = tf.get_variable('b_r', [self.state_size, ], initializer=tf.zeros_initializer()) 
            ...
            b_o = tf.get_variable('b_o', [self.state_size, ], initializer=tf.zeros_initializer()) 
    ...
    with tf.Graph().as_default():
        with tf.variable_scope("test_gru_cell"):
            ...
            with tf.variable_scope("gru"):
                tf.get_variable("W_r", initializer=np.array(
                    np.eye(3, 2), dtype=np.float32))
                tf.get_variable("U_r", initializer=np.array(
                    np.eye(2, 2), dtype=np.float32))
                tf.get_variable("b_r",  initializer=np.array(
                    np.ones(2), dtype=np.float32))
                tf.get_variable("W_z", initializer=np.array(
                    np.eye(3, 2), dtype=np.float32))
                tf.get_variable("U_z", initializer=np.array(
                    np.eye(2, 2), dtype=np.float32))
                tf.get_variable("b_z",  initializer=np.array(
                    np.ones(2), dtype=np.float32))
                tf.get_variable("W_o", initializer=np.array(
                    np.eye(3, 2), dtype=np.float32))
                tf.get_variable("U_o", initializer=np.array(
                    np.eye(2, 2), dtype=np.float32))
                tf.get_variable("b_o",  initializer=np.array(
                    np.ones(2), dtype=np.float32))
